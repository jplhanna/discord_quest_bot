...
bot = Bot(command_prefix="~", intents=default_intent, owner_id=DISCORD_OWNER_ID)  # type: ignore[arg-type]
...
@bot.hybrid_command(name="complete-quest", help="Complete an available quest")  # type: ignore[attr-defined]
async def completed_quest(ctx: Context, *, quest_name: str) -> None:
    res = complete_quest_for_user(ctx, quest_name)
    await ctx.send(res)
